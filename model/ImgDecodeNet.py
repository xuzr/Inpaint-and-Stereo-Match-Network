...
class ImgDecodeNet(nn.Module):
    def __init__(self, output_nc, ngf=32, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        self.outerdecode = nn.Sequential(
            ...
            nn.Conv2d(ngf * 2 * 3, ngf, kernel_size=1, stride=1, bias=use_bias),
            nn.ConvTranspose2d(ngf, ngf, kernel_size=4, stride=2, padding=1, bias=use_bias),
            norm_layer(ngf* 1),)
