...
def test():
    ...
    for batch_idx, (imgl, imgr, imglnoh, imgrnoh,depthl,depthr) in enumerate(test_loader):
        ...
        depthl_predNP= depthl_pred.cpu().detach().numpy().squeeze()
        depthr_predNP= depthr_pred.cpu().detach().numpy().squeeze()
        depthlNP = depthl.cpu().detach().numpy().squeeze()
        depthrNP = depthr.cpu().detach().numpy().squeeze()
